...
func rename_tag(proxy_name string,lock *sync.RWMutex,country_map map[string]int) string {
	...
	var tag_name string
	// 获取模板信息
	countries,err := sing2catconfig.Get_value("template","country")
	if err != nil{
		sing2catconfig.Logger_caller("Get country failed!",err)
		panic("Get Template failed!")
	}
	for index,country := range(countries.([]interface{})){
		// 创建正则匹配式
		...
		if reg.MatchString(proxy_name){
			...
			// 用索引数判断是否遍历到最后
			if index == len(countries.([]interface{})) - 1{
...
func Format_proxy(proxy_map map[string]interface{},lock *sync.RWMutex,country_count map[string]int) (map[string]interface{},error){
	...
	switch protocol_type {
		case "vmess":
			// 获取模板信息
			proxy_vmess,err := sing2catconfig.Get_value("template","outbounds","vmess")
			if err != nil{
				sing2catconfig.Logger_caller("Get Template failed!",err)
				panic("Get vmess template failed!")
			}
		...
		case "ss":
			// 获取模板信息
			proxy_ss,err := sing2catconfig.Get_value("template","outbounds","ss")
			if err != nil{
				sing2catconfig.Logger_caller("Get Template failed!",err)
				panic("Get ss temlpate failed!")
			}
			tag_name := rename_tag(proxy_map["name"].(string),lock,country_count)
		...
		case "trojan":
			// 获取模板信息
			proxy_trojan,err := sing2catconfig.Get_value("template","outbounds","trojan")
			if err != nil{
				sing2catconfig.Logger_caller("Get Template failed!",err)
				panic("Get trojan template failed!")
			}
