...
func format_url() ([]interface{},error) {
	// 读取配置文件获得url列表
	urls,err := sing2catconfig.Get_value("config","url")
	if err != nil {
		sing2catconfig.Logger_caller("Get url msg failed!",err)
		return nil,err
	}
	...
	for index, url := range urls.([]interface{}) {
		// 创建正则匹配器,根据?或者&切割
		...
		for _, paparameter := range parameters {
			...
			if clash_tag {
				...
				if index == len(parameters)-1 {
					urls.([]interface{})[index] = url.(string)+ "&flag=clash"
				}
			}
		}
	}
	// 返回检查后的url
	return urls.([]interface{}),nil
}
...
func select_outbound(tags []string) (map[string]interface{},error){
	// 获取模板信息
	select_map,err := sing2catconfig.Get_value("template","outbounds","select")
	if err != nil{
...
func auto_outbound(tags []string) (map[string]interface{},error){
	// 获取模板信息
	auto_map,err := sing2catconfig.Get_value("template","outbounds","auto")
	if err != nil{
...
func Merge_outbounds() ([]map[string]interface{},error){
	...
	// 获取配置文件
	rule_sets,err := sing2catconfig.Get_value("config","rule_set")
	if err != nil{
		sing2catconfig.Logger_caller("Get ruleset msg failed!",err)
		return proxies,err
	}
	for _,rule := range(rule_sets.([]interface{})){
	...
	// 获取模板信息
	custom_outbounds,err := sing2catconfig.Get_value("template","outbounds","custom_outbound")
	if err != nil{
		sing2catconfig.Logger_caller("Get custom_outbounds failed!",err)
		return proxies,err	
	}
	// 遍历自定义出站信息并添加
